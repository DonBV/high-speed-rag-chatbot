...
def get_vectorstore(text_chunks):
    ...
    if text_chunks is None:
        return PGVector(connection=connection, embeddings=embeddings, use_jsonb=True)
    return PGVector.from_texts(
        texts=text_chunks, embedding=embeddings, connection=connection
    )
...
def get_conversation_chain(vectorstore):
    ...
    llm = ChatBedrock(
        model_id="anthropic.claude-3-sonnet-20240229-v1:0", client=BEDROCK_CLIENT
    )
...
def main():
    ...
    st.image(
        image,
        caption="Generative AI Q&A with Amazon Bedrock, Aurora PostgreSQL and pgvector",
    )
